...
class MultiOutputModel(nn.Module):
    def __init__(self, device: int, n_age_cat: int, n_gender_cat: int, n_emotion_cat: int) -> None:
        ...
        # --------- Output Layers -----------------
        feat_in_emo = 1152
        feat_in_classifer_emo = 64
        self.emo_out = nn.Sequential(
            nn.BatchNorm1d(feat_in_emo),
            nn.Linear(feat_in_emo, feat_in_classifer_emo),
            nn.BatchNorm1d(feat_in_classifer_emo),
            nn.LeakyReLU(0.2, inplace=True),
            nn.Linear(feat_in_classifer_emo, n_emotion_cat)
        )
    ...
    def forward(self, context, target, pose_masks):
        ...
        out_emotion = torch.cat([pg_target_feat, sc_feat], 1)
        out_emotion = self.emo_out(out_emotion)
